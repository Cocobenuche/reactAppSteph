...
class MeteoNantes extends React.Component {
    ...
    handleChangeInput(e) {
        ...
        console.log(this.state.lienVille);
        //fetch(this.state.lienVille).then((data) => {console.log(data);
           // data.json().then((data) => {this.setState({ listeVilles: data })})});
           fetch(this.state.lienVille)
           .then((data) => { console.log(data); data.json().then((data) => { console.log(data) }) });
    ...
    render() {
        ...
        return (
            <div>
                ...
                />
                <li>
                    <button onClick={() => this.handleSubmit()}>{this.state.listeVilles}</button>
                </li>
